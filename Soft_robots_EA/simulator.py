...
class Simulator_v1:
    ...
    def run_par(self,iterations,pop):
        window = ti.ui.Window("Simulator", (768, 768),show_window=True)
        canvas = window.get_canvas()
        scene = ti.ui.Scene()
        camera = ti.ui.Camera()
        camera.position(5, 5, 5) # set camera position
        camera.lookat(0, 0, 0)      # set camera lookat
        camera.up(0, 0, 1)          # set camera up vector
        ...
        for _ in range(iterations):
            ...
            # x_cm_prev = np.mean(prev_pos[self.active],axis=0)
            for _ in range(500):
                self.step_ti(T)
                T += self.dt
            ...
            # x_cm_new = np.mean(self.pos.to_numpy()[self.active],axis=0)
            # dist = np.linalg.norm(x_cm_prev[:2]-x_cm_new[:2])
            # print(dist)
...
class Simulator_v2:
    ...
    def initialize(self):
        ...
        self.mass.fill(p_mass)
        self.springs.from_numpy(self.edges)
        self.pos.from_numpy(self.coords)
        self.springs_L0.from_numpy(self.L0)
        self.vel.fill(0.0)
        self.Force.fill(0.0)
        self.T = 0.0
    ...
    def run(self,iterations:int):
        x_cm_prev = np.sum(self.pos.to_numpy()[self.active],axis=0)/self.n_particles
        for _ in range(iterations):
            self.step_ti(self.T)
            self.T += self.dt
        x_cm_new = np.sum(self.pos.to_numpy()[self.active],axis=0)/self.n_particles
        dist = np.linalg.norm(x_cm_prev[:2]-x_cm_new[:2])
        return  dist
    
    def run_par(self,iterations:int,pop:int):
        prev_pos = self.pos.to_numpy()
        for _ in range(iterations):
            self.step_ti(self.T)
            self.T += self.dt
