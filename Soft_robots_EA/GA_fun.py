...
#Helper functions
#-------------------------------------------------------
def assemble_identical_pop(robot: np.ndarray,
                           coords: np.ndarray,
                           edges: np.ndarray,
                           pop_num:int,
                           num_material:int=10)-> List[np.ndarray,np.ndarray,np.ndarray]:
    '''
    Assemble identical population, for testing perposes only
    '''
    #mat : k , b, c
    mat1 = np.array([1000,0,0],dtype=np.float32)
    mat2 = np.array([20000,0,0],dtype=np.float32)
    mat3 = np.array([5000,0.1,0],dtype=np.float32)
    mat4 = np.array([5000,0.1,np.pi],dtype=np.float32)
    mat5 = np.array([0,0,0],dtype=np.float32)

    material = [mat1,mat2,mat3,mat4,mat5]
    unique_material = [mat1,mat2,mat3,mat4,mat5]
    material = unique_material * (num_material//5)
...
@jit(nopython = True)
def find_closest_center(main_center:np.ndarray, 
                        other_centers:np.ndarray)->int:
    '''
...
#GA functions
#------------------------------------------------------------------------
def create_pop(num:int,
               coords:np.ndarray,
               num_material:int=5)->np.ndarray:
    '''
...
def assemble_pop(population:np.ndarray,
                 coords:np.ndarray,
                 edges:np.ndarray,
                 num_material:int)->List[np.ndarray,np.ndarray,np.ndarray]:
    '''
...
def eval_population(spring_sim,
    ...
    if parallel_eval:
        ...
        dist=spring_sim.run_par(10000,pop=k_springs.shape[0])  
...
@jit(nopython = True)
def select_tour(population:np.ndarray,
                score:np.ndarray,
                tour_size:int,
                p_size:int,
                el_size:int,
                num_material:int)->List[np.ndarray,np.ndarray]:
    '''
...
@jit(nopython = True)
def variation(population:np.ndarray,
              m_rate:float,
              c_rate:float,
              num:int,
              num_material:int) ->np.ndarray:
...
def GA(domain:Union[str,tuple],
       ...
       num_material:int = 10)-> np.ndarray:
    ...
    population = create_pop(num=population_num,
    ...
    #Initilize simulator
    Mass_spring_simulator = Simulator_v2(**sim_structure)
    #Initialize logger 
